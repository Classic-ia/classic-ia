 ...
 * Baseado no cq_auth.js — adaptado para o sistema RH
 */
...
const RHAuth = (function () {
  ...
  async function _abrirSessao(refreshToken) {
    ...
    try { await _post('rh_sessoes', {
      ...
      expira_em:     new Date(Date.now() + TIMEOUT_MS).toISOString(),
      ativa:         true,
      criado_em:     new Date().toISOString(),
    }); } catch {}
  ...
  async function _heartbeat() {
    ...
    await _patch('rh_sessoes',
      ...
      { ultimo_acesso: new Date().toISOString(), expira_em: new Date(Date.now() + TIMEOUT_MS).toISOString() }
    );
  }
  ...
  function _mostrarAviso() {
    ...
    document.getElementById('_rh_warn_btn').onclick = () => { _onAtividade(); _heartbeat(); };
  }
  ...
  return { init, login, logout, log, getUser, getToken, getAuthToken };
})();
