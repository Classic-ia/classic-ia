/**
 * auth.js — JR Terraplanagem
 * Autenticacao via Supabase Auth + tabela jr_usuarios (RBAC).
 * Sessao isolada do Trans Anacleto (chave jr_sess_v1 em localStorage).
 */

const JrAuth = (function () {
  ...
  async function _revalidarPerfil() {
    try {
      const r = await fetch(`${SB_URL}/rest/v1/rpc/jr_meu_perfil`, {
  ...
  function _onAtividade() {
    _resetTimers();
    const el = document.getElementById('_jr_warn');
    if (el) el.remove();
  }

  function _mostrarAviso() {
    if (document.getElementById('_jr_warn')) return;
    ...
    el.innerHTML = `
      ...
      <button id="_jr_warn_btn" class="rh-session-warn-btn">Continuar</button>`;
    document.body.appendChild(el);
    document.getElementById('_jr_warn_btn').onclick = () => { _onAtividade(); };
  }
  ...
  async function login(email, senha) {
    ...
    _user = {
      ...
      perfil: data.user.user_metadata?.perfil || 'operador',
    };
    const perfilDB = await _revalidarPerfil();
    if (perfilDB) {
      _user.id = perfilDB.id; _user.nome = perfilDB.nome;
      ...
      throw new Error('Usuario nao cadastrado ou desativado no modulo JR Terraplanagem. Contate o administrador.');
    }
    _token = _mkToken();
    localStorage.setItem(STORE_KEY, JSON.stringify({
      token: _token, user: _user,
