/**
 * auth.js — Trans Anacleto
 * Autenticacao via Supabase Auth + tabela trans_usuarios (RBAC).
 * Sessao isolada do modulo RH/CQ (chave propria em localStorage).
 */

const TransAuth = (function () {
  ...
  function _onAtividade() {
    _resetTimers();
    const el = document.getElementById('_trans_warn');
    if (el) el.remove();
  }

  function _mostrarAviso() {
    if (document.getElementById('_trans_warn')) return;
    ...
    el.innerHTML = `
      ...
      <button id="_trans_warn_btn" class="rh-session-warn-btn">Continuar</button>`;
    document.body.appendChild(el);
    document.getElementById('_trans_warn_btn').onclick = () => { _onAtividade(); };
  }
  ...
  async function login(email, senha) {
    ...
    _user = {
      ...
      perfil: data.user.user_metadata?.perfil || 'almoxarife',
    };
    const perfilDB = await _revalidarPerfil();
    if (perfilDB) {
      _user.id = perfilDB.id; _user.nome = perfilDB.nome;
      ...
      throw new Error('Usuario nao cadastrado ou desativado no modulo Trans Anacleto. Contate o administrador.');
    }
    _token = _mkToken();
    localStorage.setItem(STORE_KEY, JSON.stringify({
      token: _token, user: _user,
